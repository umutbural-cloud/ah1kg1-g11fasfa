...
import { Plus, GripVertical, ArrowRight, Clock } from "lucide-react";
...
const KanbanColumn = ({ column, tasks, onCreateTask, onUpdateTask, onDeleteTask, onOpenTask }: {
      ...
      <div className="space-y-2 flex-1 min-h-[100px]">
        ...
        <SortableContext items={visibleTasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
          {visibleTasks.map((task) => (
            <SortableCard key={task.id} task={task} onUpdate={onUpdateTask} onDelete={onDeleteTask} onOpen={onOpenTask} />
          ))}
        </SortableContext>
